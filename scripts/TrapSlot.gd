...
func _find_background_panel() -> void:
	background_panel = get_node_or_null("Panel") as Control
...
func _on_slot_pressed() -> void:
	...
	if manager == null:
		push_error("TrapSlot: TrapManager not found.")
		return
...
func _complete_tutorial_if_needed() -> void:
	...
	if not tutorial_ui.is_trap_placement_step():
		return

	if not tutorial_ui.is_valid_trap_target(self):
		return

	tutorial_ui.complete_trap_placement()
...
func get_world_position() -> Vector2:
	...
	if gameplay != null:
		var trap_positions_node := gameplay.get_node_or_null(
			"World/TrapPositions"
		)
		...
		if trap_positions_node != null:
			...
			var marker := trap_positions_node.get_node_or_null(
				marker_name
			) as Marker2D
